    ...
    try {
      ...
      if (data && data[0]) {
        ...
        if (altText && altText !== text) {
          console.log("Alternative translation result:", {
            original: text,
            translated: altText,
          });
          return altText;
      ...
      console.warn(
        "Translation extraction failed, returning original text:",
        text,
      );
      return text;
    } catch (error) {
    ...
    if (!recognitionRef.current) {
      ...
      recognition.onerror = (error) => {
        ...
        if (errorType === "not-allowed") {
          ...
          toast.error(
            "Speech recognition service is not available. This may be due to browser settings or network restrictions. Please use text input instead.",
          );
          ...
          toast.error(
            `Speech recognition unavailable: ${errorMsg}. Please use text input instead.`,
          );
        }
      };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (
        ...
        !reportData.coordinates[0] ||
        !reportData.coordinates[1]
      ) {
      ...
      try {
        ...
        mlResult = await apiService.validateReportWithML(
          mlPayload,
          selectedFile,
          45000,
          2,
      ...
      if (mlResult && mlResult.category) {
        ...
        if (mlResult === null) {
          console.log(
            "ML validation skipped (timeout/unavailable), using default category",
          );
        }
      }
      ...
      if (mlResult && mlResult.priority) {
        issueData.priority =
          mlResult.priority === "urgent" ? "urgent" : "medium";
      }
          ...
          >
            <button
              ...
              style={{
                ...
                padding: "1rem",
                background:
                  recordingType === "photo" ? "#1e4359" : "transparent",
              ...
              <button
                ...
                title={
                  isSpeechRecognitionAvailable
                    ? "Dictate title with your voice"
                    : "Speech recognition not available. Please use text input."
                }
                style={{
                  minWidth: "120px",
                  opacity: isSpeechRecognitionAvailable ? 1 : 0.5,
                  cursor: isSpeechRecognitionAvailable
                    ? "pointer"
                    : "not-allowed",
                }}
              >
                {isListening && targetRef.current === "title"
                  ? "Stop"
                  : "Speak"}
              </button>
            </div>
