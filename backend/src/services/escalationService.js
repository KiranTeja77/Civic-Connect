...
class EscalationService {
  ...
  async checkAndEscalateIssues() {
    try {
      const now = new Date();
      console.log(
        `[ESCALATION] Checking for overdue issues at ${now.toISOString()}`,
      );
        ...
        console.log(
          ...
          overdueIssues.map((issue) => ({
            ...
            hoursOverdue: issue.escalationDeadline
              ...
              : Math.round(
                  (now - (issue.assignedAt || issue.createdAt)) /
                    (1000 * 60 * 60),
                ),
          })),
        );
      ...
      for (const issue of overdueIssues) {
        try {
          console.log(
            `[ESCALATION] Attempting to escalate issue ${issue._id} from ${issue.assignedRole}`,
          );

          // For old issues without assignedRole, determine appropriate role based on age
          if (!issue.assignedRole) {
            console.log(
              `[ESCALATION] Issue ${issue._id} has no assignedRole. Determining appropriate role based on age.`,
            );
            ...
            if (priority === "high" || priority === "urgent") {
              if (hoursOld >= 1 / 6) {
                // 5min + 5min = commissioner level (1/6 hour = 10 minutes)
                targetRole = "commissioner";
              } else if (hoursOld >= 1 / 12) {
                // 5min = supervisor level (1/12 hour = 5 minutes)
            ...
            console.log(
              `[ESCALATION] Issue ${issue._id} is ${Math.round(hoursOld)}h old (${priority} priority). Assigning to ${targetRole}.`,
            );
            ...
            if (targetRole === "field-staff") {
              ...
              if (fieldStaffResult) {
                console.log(
                  `[ESCALATION] ✅ Assigned issue ${issue._id} to field-staff`,
                );
              ...
              const targetUsers = await this.findAllUsersForRole(
                targetRole,
                issue.category,
              );
              if (targetUsers && targetUsers.length > 0) {
                ...
                if (issue.priority) {
                  issue.escalationDeadline = issue.calculateEscalationDeadline(
                    issue.priority,
                    targetRole,
                  );
                }
                ...
                const notificationPromises = targetUsers.map((user) =>
                  notificationService.notifyIssueAssignment(
                    issue,
                    user,
                    targetUsers[0],
                  ),
                ...
                console.log(
                  `[ESCALATION] ✅ Fast-tracked issue ${issue._id} directly to ${targetRole}`,
                );
                escalationResults.push({
                  issueId: issue._id,
                  title: issue.title,
          ...
          if (result) {
            console.log(
              `[ESCALATION] ✅ Successfully escalated issue ${issue._id} to ${result.toRole}`,
            );
            escalationResults.push({
              issueId: issue._id,
              title: issue.title,
          ...
          console.error(
            `[ESCALATION] ❌ Error escalating issue ${issue._id}:`,
            error,
          );
          escalationResults.push({
            issueId: issue._id,
  ...
  async escalateIssue(issue) {
    ...
    const nextRoleUsers = await this.findAllUsersForRole(
      nextRole,
      issue.category,
    );
    ...
    if (!issue.escalationDeadline && issue.priority && currentRole) {
      console.log(
        `[ESCALATION] Issue ${issue._id} missing deadline. Setting deadline for ${currentRole} before escalation.`,
      );
    ...
    await issue.escalate(
      nextRole,
      assignedUser._id,
      "Auto-escalated: Time limit exceeded",
    );
    ...
    console.log(
      `✅ Issue ${issue._id} escalated to ${nextRole}. ${nextRoleUsers.length} ${nextRole}(s) notified.`,
    );
  ...
  async assignToFieldStaff(issue) {
    const fieldStaff = await this.findUserForRole(
      "field-staff",
      issue.category,
    );
  ...
  async findUserForRole(role, category) {
    ...
    if (!user) {
      user = await User.findOne({
        role,
        isActive: true,
        $or: [{ departments: "All" }, { department: "All" }],
      }).sort({ loginCount: 1, lastLogin: -1 });
    }
  ...
  async manualEscalate(issueId, toRole, escalatedBy, reason) {
    ...
    await notificationService.notifyIssueAssignment(issue, assignedUser, {
      _id: escalatedBy,
    });
