      ...
      if (category && category !== "all") {
        if (filter.$or) {
          ...
          filter.$or = filter.$or.map((condition) => {
            if (condition.category && Array.isArray(condition.category.$in)) {
              ...
              return {
                ...condition,
                category: {
                  $in: condition.category.$in.filter((c) => c === category),
                },
              };
            }
            return {
          ...
          if (filter.category && Array.isArray(filter.category.$in)) {
            ...
            filter.category = {
              $in: filter.category.$in.filter((c) => c === category),
            };
          } else {
    ...
    try {
      ...
      if (
        ...
        return res.status(403).json({
          success: false,
          message:
            "This issue is assigned to another employee. Only the assigned employee can accept it.",
        });
      }
      ...
      if (result.matchedCount === 0) {
        ...
        if (currentIssue.acceptedBy) {
          return res.status(400).json({
            success: false,
            message:
              "This issue has already been accepted by another employee.",
          });
        }
        ...
        if (
          ...
          currentIssue.assignedTo.toString() !== user._id.toString()
        ) {
          return res.status(403).json({
            success: false,
            message:
  ...
  async resolveIssue(req, res) {
    try {
      ...
      const isAcceptedByUser =
        issue.acceptedBy?.toString() === user._id.toString();
      ...
      if (issue.acceptedBy) {
        if (!isAdmin && !isAcceptedByUser) {
          return res.status(403).json({
            success: false,
            message:
              "Only the employee who accepted this issue can resolve it.",
          });
        }
      } else {
        ...
        if (issue.reportedBy && !issue.pointsAwarded) {
          try {
            ...
            if (reporter) {
              ...
              console.log(
                `Awarded +10 points to user ${reporter._id} for resolved issue ${issue._id}. New total: ${reporter.points}`,
              );
            }
          } catch (pointsError) {
      ...
      console.log(
        `[RESOLVE] Attempting to remove issue ${issue._id} from ML dataset. reportId: ${issue.reportId}, ML_API_URL: ${process.env.ML_API_URL ? "configured" : "not configured"}`,
      );
      ...
      if (process.env.ML_API_URL) {
        try {
          ...
          console.log(
            `[RESOLVE] Calling ML remove endpoint: ${removeUrl} with report_id: ${issue.reportId || "NOT SET"}`,
          );
          ...
          if (reportIdToRemove) {
            ...
            console.log(
              `[RESOLVE] Using reportId to remove: ${reportIdToRemove}`,
            );
          } else {
            // Fallback: use description and user_id
            console.log(
              `[RESOLVE] Issue ${issue._id} does not have reportId. Using fallback: description/user_id...`,
            );
            if (issue.description && issue.reportedBy) {
              // Handle both populated object and ObjectId
              const userId = issue.reportedBy._id
                ? issue.reportedBy._id.toString()
                : issue.reportedBy.toString();
              params.append("description", issue.description);
              params.append("user_id", userId);
              console.log(
                `[RESOLVE] Fallback params: description="${issue.description.substring(0, 50)}...", user_id="${userId}"`,
              );
            } else {
              console.warn(
                `[RESOLVE] Cannot remove issue ${issue._id} from ML dataset: missing reportId, description, or reportedBy`,
              );
            }
          }
          ...
          if (params.toString()) {
            ...
            console.log(
              `[RESOLVE] ML remove response status: ${removeResponse.status}, body: ${responseText}`,
            );
            ...
            if (removeResponse.ok) {
              try {
                ...
                console.log(
                  `[RESOLVE] ✅ Successfully removed issue ${issue._id} from ML dataset:`,
                  removeResult,
                );
              } catch (parseError) {
                console.log(
                  `[RESOLVE] ✅ Removed issue ${issue._id} from ML dataset (non-JSON response): ${responseText}`,
                );
              }
            } else {
              console.error(
                `[RESOLVE] ❌ Failed to remove issue from ML dataset. Status: ${removeResponse.status}, Response: ${responseText}`,
              );
            }
          }
        } catch (removeError) {
          ...
          console.error(
            `[RESOLVE] ❌ Error removing issue from ML dataset (non-blocking):`,
            removeError,
          );
        ...
        console.warn(
          `[RESOLVE] ML_API_URL not configured. Cannot remove issue from ML dataset.`,
        );
      }
